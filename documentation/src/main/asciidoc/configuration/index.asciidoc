...
Now that you have Rewrite installed, and understand the basic terms, let's start writing some rules. We will start with a simple example: logging an inbound request URL to the server log.

First, open the +ConfigurationProvider+ you defined during link:install[installation], we will add the desired logging statement using the +.addRule()+ method of +ConfigurationBuilder+. This will demonstrate how to use simple parameters.

[source,java]
.Use ConfigurationBuilder to add Conditions and Operations to your ConfigurationProvider
----
...
----
You might notice, however, that the results are not quite what you expected, and that only requests with a single segment (+http://localhost:8080/segment1/+) are being logged, while requests with more than one segment (+http://localhost:8080/segment1/segmentN/+) are not.

[source,console]
.Log output
----
INFO [org.ocpsoft.config.Log] Client requested path: /foo
INFO [org.ocpsoft.config.Log] Client requested path: /bar
----

This is because, by default, +Path+ parameters match all characters except for forward slash "+/+", and if we want to change this, we must configure the parameter ourself.

[source,java]
.Modify  parameters to match a specific values using +.where()+
----
public class ApplicationConfigurationProvider extends HttpConfigurationProvider
{
...
Now we will see that our rule matches for any inbound request, with any number of path segments. This type of parameter configuration can be used any time when parameters have been specified using the +ConfigurationBuilder+

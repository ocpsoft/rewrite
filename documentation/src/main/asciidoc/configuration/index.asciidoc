...
By default, Rewrite uses a +ConfigurationCacheProvider+ that stores your configuration in the {@link ServletContext} as an application-wide caching medium. This means, by default, the +Configuration+ will be loaded once upon application startup, and never again.

To disable the default caching mechanism, and enable {@link Configuration} reloading on each request, add the following servlet context init parameter to <code>web.xml</code>:

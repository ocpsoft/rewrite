...
public class PathTest
{
   ...
   {
      Assert.assertTrue(Path.matches("/application/path").accepts(rewrite));
   }

   @Test
   public void testPathMatchesPattern()
   {
      Assert.assertTrue(Path.matches("/application/.*").accepts(rewrite));
   }

   @Test
   public void testDoesNotMatchNonHttpRewrites()
   {
      Assert.assertFalse(Path.matches("/blah").accepts(new MockRewrite()));
   }

   @Test(expected = IllegalArgumentException.class)
   public void testNullCausesException()
   {
      Assert.assertFalse(Path.matches(null).accepts(new MockRewrite()));
   }
}

...
{
   /**
    * The BeanManager may not have been initialized at the time JSF is
    * initializing. Therefore, we stick in a ForwardingELResolver that delegates
    * to the BeanManager ELResolver, which will be plugged in when it's
    * available. If the ELResolver is invoked before the BeanManager is
    * available, the resolver will perform no action (and thus produce no
    * result).
    */
   ...
   private final LazyBeanManagerIntegrationELResolver elResolver;
